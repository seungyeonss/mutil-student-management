...
public class SearchIdServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (id == null || !id.matches("\\d{9}")) {  // 학번이 9자리 숫자가 아닐 경우
            ...
            out.println("window.location.href = 'updateStudent.html';"); // 다시 검색 페이지로 이동
            out.println("</script>");
            out.close();
            return; // 메서드 종료
        }
        ...
        if (student != null) { // 학번이 존재하는 경우
            ...
            out.println("</table>");
    		out.println("<br><br>");
    		out.println("<a href = 'programList.html'> 홈으로 돌아가기 </a><br><br> ");
    		
    		
           out.println("<a href='updateStudent.do?id=" + student.getId() + "'>수정하기</a>");
            ...
            out.println("<html>");
            out.println("<body>");
            out.println("<script type=\"text/javascript\">"); //자바스크립트로 오류메시지 출력 <자바스트립트 chat gpt 이용>
            out.println("alert('일치하는 학번이 없습니다. 다시 입력하세요.');");
            out.println("location='updateStudent.html';"); // 다시 검색 페이지로 이동
            out.println("</script>");
            out.println("</body>");
            out.println("</html>");
